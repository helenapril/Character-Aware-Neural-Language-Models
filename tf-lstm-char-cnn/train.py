...
# model params
flags.DEFINE_integer('rnn_size',        325,                            'size of LSTM internal state')
flags.DEFINE_integer('highway_layers',  1,                              'number of highway layers')
flags.DEFINE_integer('char_embed_size', 75,                             'dimensionality of character embeddings')
flags.DEFINE_string ('kernels',         '[1,2,3,4,5]',                  'CNN kernel widths')
flags.DEFINE_string ('kernel_features', '[50,150,100,100,50]',         'number of features in the CNN kernel')
flags.DEFINE_integer('rnn_layers',      1,                              'number of layers in the LSTM')
flags.DEFINE_float  ('dropout',         0.2,                            'dropout. 0 = no dropout')
...
flags.DEFINE_integer('num_unroll_steps',    40,   'number of timesteps to unroll for')
flags.DEFINE_integer('batch_size',          150,   'number of sequences to train on in parallel')
flags.DEFINE_integer('max_epochs',          6,   'number of full passes through the training data')
flags.DEFINE_float  ('max_grad_norm',       5.0,  'normalize gradients at')
flags.DEFINE_integer('max_word_length',     15,   'maximum word length')
flags.DEFINE_integer('max_max_epochs',       2,   'number of full passes through the training data')
...
def main(_):
    ...
    print('initialized train/valid dataset readers')
        ...
        for epoch in range(FLAGS.max_epochs):
            ...
            for x, y in train_reader.iter():
                ...
                if count % FLAGS.print_every == 0:
                    print('%6d: %d [%5d/%5d], train_loss/perplexity = %6.8f/%6.7f' % (step,
                                                            epoch, count,
                                                            train_reader.length,
                                                            loss, np.exp(loss)))

            avg_train_loss /= count
            train_start_time += (time.time()-epoch_start_time)
            ...
            elif epoch > FLAGS.max_max_epochs:
                best_valid_loss = avg_valid_loss
                current_learning_rate = session.run(train_model.learning_rate)
                current_learning_rate *= FLAGS.learning_rate_decay
                if current_learning_rate < 1.e-10:
                    print('learning rate too small - stopping now')
                    break
                session.run(train_model.learning_rate.assign(current_learning_rate))
                print('new learning rate is:', current_learning_rate)
