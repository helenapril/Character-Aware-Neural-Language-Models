...
def load_vocab(data_dir, filename):

    dictionary = {}
    num = 0
    with codecs.open(os.path.join(data_dir, filename), 'r', encoding='gbk') as f:
        for line in f:
            line = line.split()
            if len(line) == 2:
...
def load_data(data_dir, max_word_length, eos='+'):
    ...
    word_vocab = load_vocab(data_dir, 'pplm.dict')
    ...
    for fname in ('train', 'valid', 'test'):
        print('reading', fname)
        with codecs.open(os.path.join(data_dir, fname), 'r', 'gbk') as f:
            for line in f:
                if line[0] == '1':
                    line = line[1:]
                line = line.strip()
                ...
                for word in line.split():
                    ...
                    word_tokens[fname].append(word_vocab.setdefault(word, 0))
                    if word_vocab.setdefault(word, 0) == 0:
                        word = '<unk>'
                ...
                if eos:
                    word_tokens[fname].append(len(word_vocab)+1)
    ...
    print('size of word vocabulary:', len(word_vocab))
